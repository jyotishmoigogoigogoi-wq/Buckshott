...
async def start_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(get_welcome_msg(), parse_mode=ParseMode.HTML)
...
async def buckshotpv_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in private_games and private_games[user_id].get('status') == 'playing':
        await update.message.reply_text("⚠️ You already have an active game!")
        return
    ...
    msg = await update.message.reply_text(reload_msg, parse_mode=ParseMode.HTML)
    ...
    await msg.edit_text(game_display, reply_markup=get_private_game_kb(user_id), parse_mode=ParseMode.HTML)
...
async def buckshot_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text(lobby_msg, reply_markup=get_lobby_kb(chat_id), parse_mode=ParseMode.HTML)
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("join_"):
        ...
        if len(game['players']) == 2:
            ...
            # Show reload
            reload_msg = get_reload_msg(live, blank)
            await query.edit_message_text(reload_msg, parse_mode=ParseMode.HTML)
            await asyncio.sleep(2)
            ...
            await query.edit_message_text(lobby_msg, reply_markup=get_lobby_kb(chat_id), parse_mode=ParseMode.HTML)
    ...
    if data == "play_again_pv":
        ...
        game_display = get_game_display(private_games[user_id], is_group=False)
        await query.edit_message_text(game_display, reply_markup=get_private_game_kb(user_id), parse_mode=ParseMode.HTML)
        return
...
async def process_private_shot(query, user_id: str, target: str):
    ...
    if game['p1_hp'] <= 0:
        ...
        await query.edit_message_text(game_over, reply_markup=get_play_again_kb(True), parse_mode=ParseMode.HTML)
        return
    ...
    if extra_turn:
        extra_msg = get_extra_turn_msg("YOU")
        await query.edit_message_text(extra_msg, parse_mode=ParseMode.HTML)
...
async def process_ai_turn(query, user_id: str):
    ...
    while game['turn'] == 2 and game['status'] == 'playing':
        # AI thinking
        await query.edit_message_text(get_ai_thinking_msg(), parse_mode=ParseMode.HTML)
        await asyncio.sleep(1.5)
        ...
        if extra_turn:
            extra_msg = get_extra_turn_msg("DEALER")
            await query.edit_message_text(extra_msg, parse_mode=ParseMode.HTML)
            await asyncio.sleep(1.5)
        else:
            break
...
async def process_group_shot(query, chat_id: str, shooter_id: int, target: str):
    ...
    if game['p1_hp'] <= 0:
        ...
        await query.edit_message_text(game_over, reply_markup=get_play_again_kb(False), parse_mode=ParseMode.HTML)
    ...
    # Extra turn message
    if extra_turn:
        extra_msg = get_extra_turn_msg(shooter_name[:10])
        await query.edit_message_text(extra_msg, parse_mode=ParseMode.HTML)
        await asyncio.sleep(1.5)
